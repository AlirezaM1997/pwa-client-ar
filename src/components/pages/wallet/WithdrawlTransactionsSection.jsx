...
// GQL
import { GET_MY_SHABA_WITHDRAWREQUESTS } from "@services/gql/query/GET_MY_SHABA_WITHDRAWREQUESTS";
// FUNCTION
...
export default function WithdrawlTransactionsSection({
  t,
  tW,
  lang,
  setDetailsData,
  ...
  const [getProjects, { error, loading, data, fetchMore }] = useLazyQuery(
    GET_MY_SHABA_WITHDRAWREQUESTS,
    {
      variables: { limit: limit_item, page: 0 },
      fetchPolicy: "no-cache",
    }
  );
  const total = data?.get_my_withdrawRequests?.total ?? 0;

  useEffect(() => {
    if (!data && page === 0) getProjects();
    if (data && page === 0) {
      setResult(data?.get_my_withdrawRequests?.result);
      setPage(1);
    }
  }, [data, page]);
  ...
  const fetchingMore = () => {
    ...
    fetchMore({ variables: { page } })
      .then(({ data }) => {
        setResult([...result, ...(data?.get_my_withdrawRequests?.result ?? null)]);
        setPage(page + 1);
      })
          ...
          {result?.map((item, index) => (
            <div
              ...
              onClick={() => {
                ...
                setDetailsData({
                  ...
                  amount: item.amount,
                  date: item.createdAt,
                  shabaNumber: item.wallet.shabaNumber,
                  cartNumber: item.wallet.cartNumber,
            ...
            >
              <TransactionCard
                ...
                isFailed={String(item.status).toUpperCase() === "REJECT"}
                date={item.createdAt}
