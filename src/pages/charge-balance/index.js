...
// GQL
import { VALIDATE_ZARINPAL_CHARGE_BALANCE } from "@services/gql/mutation/VALIDATE_ZARINPAL_CHARGE_BALANCE";
...
export default function AfterPaymentPage() {
  ...
  const [validateZarinpalChargeBalance] = useMutation(VALIDATE_ZARINPAL_CHARGE_BALANCE);
  ...
  const callbackFunc = () => {
    ...
    if (token._id) {
      validateZarinpalChargeBalance({ variables });
    }
  };
