...
export default function ParticipationPage({ params }) {
  ...
  return (
    <>
      <div className="px-4 2xl:max-w-[1320px] 2xl:m-auto">
        <Head>
          <title>{tPA("myParticipations.donateList")}</title>
        </Head>
        <header className="pt-5 pb-[22px] relative lg:hidden">
        ...
        <Header
          hasBackButton={false}
          title={tPA("myParticipations.donateList")}
        ...
        <InfiniteScroll
          ...
          className={
            participations?.length === 0
              ? ""
              : "flex flex-col gap-[20px] mb-[48px] lg:grid lg:grid-cols-2 lg:mt-[47px]"
          }
        >
          {participations.map((item, index) => {
            return <ReceiptCard t={t} tPA={tPA} receipt={item} key={"ParticipationCard" + index} />;
          })}
